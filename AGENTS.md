...
**Cone（`conelevel.md:70–86`）**——不含地址，因为客户端无法预知受托服务器：
```
Rnd16[0] = Rnd16[0] & 0x3E | from   // from: 0=NewPort, 1=NewHost
SN = Rnd16 + HMAC_SHA256(Key, Rnd16 + TmpN) + TmpN
```

**Keepalive（`keepalive.md:62–76`）**——含对端地址，可互相验证：
```
Rnd16[0] &= 0x3F                       // 仅清高 2 位，无 NewPort/NewHost 标志
...
**Cone**（`conelevel.md:121–127, 194–196`）：服务端发 **3** 次，间隔 `200ms, 400~800ms`，累计≤1s；客户端超时 **5 秒**。

**Keepalive**（`keepalive.md:87–91, 109`）：服务端发 **9** 次，间隔 `100, 200, 400, 800, 1600, 1600, 1600, 1600ms`，累计约 7.9s；客户端超时 **10 秒**。
...
- 精测按二分规则逼近，示例精度 **5s**（`keepalive.md:131`），可配置。
- 起始间隔不低于 **10s**。
