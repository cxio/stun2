...
使用新的随机端口向同一服务节点拨号，创建一个新的 QUIC 安全连接，然后请求存活期探测（`STUN:Live`）服务。

发送数据为：

- 起始计数（`Start`）：服务器冗余发送的消息包在此基础上递增计数，表达发送的顺序。
- 序列号标识因子（`Bas16`）：16随机字节序列，用于序列号生成（规则如前 *哈希耦合*）。
- 旧端口（`Port.0`）：即之前请求创建的NAT映射（旧链路）的端口号。

与此同时，客户端会在旧链路（纯UDP）上发送 PING 包以维持原先的 NAT 映射。

> **注：**
> 旧链路上的 PING 包也是下一次 `STUN:Live` 探测请求的计时重置（回到零点）时机。


#### 服务器操作

1. 提取客户端的来源IP，以及传递来的旧端口（`Port.0`），构造客户端原公网映射地址。
2. 在旧链路的连接（net.UDPConn）上向客户端地址发送消息，数据构造规则为*哈希耦合*（见前NAT类型探测部分）。

> **注记：**
> 服务器在旧链路上的回应是一种只写逻辑，无需理会客户端发送的 PING 包。

考虑原生纯UDP链路的不可靠性，服务器每收到一个 `STUN:Live` 请求，会发送3个消息包（冗余），每个包相隔 100ms。

> **实现：**
> 冗余消息包里的计数（`Start`）会递增，且该值由 `Rnd16` 的首个字节表达。
> 因此该起始值实际上不应太大（如 <250），每次随机生成，避免特征。
...
> **注：**
> 客户端需要对收到的序列号消息进行验证，以确保消息是由服务器发送而来，而非伪造。
...
如果探测超时（NAT 映射失效），则客户端需从**Step.0**开始一个全新的探测流程，即从 *粗测* 到 *精测* 的转换。新的流程无需在原来的服务器上继续，粗测和精测是可以完全独立（分离）的。


#### 粗测

客户端每隔一段时间发起一次请求（`STUN:Live`），时间间隔为**15秒**增量（即精度）。

初始从 `20s` 开始，然后间隔 `35s`、`50s`、`65s` ...，直到设定的终点（最长间隔）或超时。

> **注意：**
> 间隔时间完全由客户端控制，服务器仅收到 `STUN:Live` 请求后才发送探测包。
> 起始间隔不应小于某个值（如10秒），以让双方可以干净地退出 QUIC。
