# STUN2 -- NAT「地址映射/入站过滤/状态存活期」探测库

> STUN: Session Traversal Utilities for NAT

不同于传统的 STUN（RFC 3489），本项目设计了一种新的探测流程来获知 NAT 内网节点的 NAT 相关信息。

这一流程基于*STUN服务节点是一个P2P网络*的环境，服务节点间可相互协作，便捷地提供不同 IP 的回包，同时也简化了探测流程。


## NAT 类型

一台连入公网的节点可能是直接的公网主机，也可能是内网中通过 NAT 网关/防火墙连入公网的主机。

公网主机可接收任意来源的连入请求，但 NAT 内网主机就有区别了。这与网关的 NAT 映射方式和防火墙过滤规则有关，此统称为 NAT 类型。

常见的 NAT 类型有：

1. `FullC`：完全圆锥型（`Full Cone`）。最宽松，网关不对外部来源地址（IP:Port）作任何限制，可接收任意地址的数据包进入内网。
2. `RC`：受限圆锥型（`Restricted Cone`）。外部连入的源IP需与内网连出时的目标IP相同，端口任意。
3. `P-RC`：端口受限圆锥型（`Port Restricted Cone`）。除了IP要一致外（`RC`），外部连入的源Port必须等于内部连出的目标Port（更严过滤）。
4. `Sym-Like`：类对称型（`Symmetric Like`）。内网主机连出的每一个不同目标都会新建一个不同的NAT映射，外部端口没有确定性。

另外还有几个不划归到 NAT 类型但与 UDP 通讯相关的网域类型：

1. `Public`: 开放网络（`Open Internet`）。没有任何约束的公网地址，支持任何连入。
2. `UDP Firewall`: 有UDP防火墙。仅主动连出的目标可以与内网节点通讯，即：先有连出，后可连入。
3. `UDP Blocked`: UDP阻塞。可能 `UDP:53` 等系统级消息正常，但无法用于普通的UDP通讯。
4. `QUIC Only`: 仅允许 UDP 链路的 QUIC 数据流通过，普通 UDP 被阻断。


## 可连通性

- `FullC` 类似于公网 `Public`，但需要连接保活（keep-alive），可以接受任意节点的连入。
- `Sym-Like` 对每个目标的 NAT 映射都不同，无法复用端口导致无法提供打洞信息，因此基本上不支持打洞。
- `UDP Firewall` 和 `P-RC` 相似，只要先发出了请求，就可以接受目标的回应。`RC` 则更为宽松。
- `QUIC Only` 底层 UDP 链路，可以使用 QUIC 协议通讯，打洞也需要 QUIC 流量。半 UDP 可用性。

> **提示：**
> 一个仅受限于家庭网关的公网节点可通过 UPnP 映射成为公网节点。


## 项目结构

项目包含完整3个部分：

1. **基础库**：可脱离上层应用逻辑的公共库部分，定义基础的结构、格式和规范。
2. **客户端**：实现探测功能需要的客户端的行为规范、指导和建议。
3. **服务器**：实现探测服务需要的服务器端的行为规范、指导和建议。


## 用法

（待实现完成后补充）
