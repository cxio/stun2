...
- `conception/keepalive.md`：Step.1 只关 `quic.Conn`；首次 `Time.0` 在关闭残留发送结束之后；旧链路从 `Time.0` 起双方静默；观测 Conn 进入 closing 后除当场一个 `CONNECTION_CLOSE` 外对 Address 禁发直至下次 `STUN:Live`；后续 `Time.0` 为对 SN 的最后回包时刻；复用旧 Socket 读；探测包从 Listener 同一 IP:Port 发出；每一次静默间隔 ≥ 10s（低于 10s 则按 10s 测一次）；精测精度为收敛终止误差。
