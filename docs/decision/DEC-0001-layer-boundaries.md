...
| 包 | 职责 | 不含 |
|----|------|------|
| `stun2` | 编解码与纯函数：SN、HMAC/时间窗、信封、地址规范化、判定矩阵、协议常量；Cone 挑战调用 `github.com/cxio/equix-cgo/puzz` | 网络 IO、计时、重试、节点发现、Equi-X 算法 |
...
3. **驱动模型。** `stun2/client` 以可单步推进的状态机为核心，另提供带 `context.Context` 的 Runner。应用默认走 Runner；测试、暂停或自行调度时推进状态机。库不把粗测进度持久化。
...
8. **配置分档。** 构想标明不可配置的硬编码；标明可配置的由库给默认值、调用方可覆盖；粗测起始间隔无默认值，调用方必填。静默间隔硬性下限见构想。
