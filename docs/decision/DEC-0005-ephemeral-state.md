...
四项都是协议能走完的最小记忆。Challenge 键来自客户端带回的 Proofs，与换 Socket 兼容。限速键用被测 Address，与「SN 打向谁」一致，也限制 Validation 持票后对单一映射的发包频率。受托只保留 Target 去重表：表随挑战有效期过期，Equi-X 又把证明绑定到 Target，重放被限制在短窗口、固定目标、每目标一次。

## 影响 （Consequences）

- Spec 不得引入「Inquire 会话 ID」「按控制通道 IP 的 Live 会话」等额外表。
- 受托去重表与限速表的容量、驱逐策略由 Spec 规定，但不得改键。
