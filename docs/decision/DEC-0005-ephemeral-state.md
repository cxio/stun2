...
挑战集按 Challenge 索引、TTL 不超过 2 分钟，以及 Live 探测包发往请求里的 `Address`、不绑控制通道 IP，构想已经写明。本决策只补「还允许哪些表、键是什么」。

## 决策 （Decision）

服务端允许的短暂存储是闭集，只含下列五项。此外不得为「会话」再建模。
...
3. **Live：最小间隔表。** 实现构想「同一目标地址 10s 最小间隔」。键为请求中的目标 **Address**（被测映射），不是控制通道上观测到的 IP:Port；记录上次接受该 Address 的时间，满 10s 可再接受。控制通道因 Arbitrary 池化可以与 Address 不同：按控制通道限速既防不住对同一映射的连打，也会误伤多 Socket 客户端。
...
5. **Cone 源：Inquire 客户端地址暂存。** 实现 DEC-0007。键为 Inquire 控制通道对端地址（去端口：IPv4 / IPv4-mapped 为 32 位，IPv6 为 `/64`）；值为到期时间；有效期 30s，不可配置。项仍在则拒新 Inquire。不是会话：不索引 Challenge，不把后续 `STUN:Cone` 绑回这条连接。
...
五项都是协议能走完的最小记忆。Challenge 键来自客户端带回的 Proofs，与换 Socket 兼容。Live 限速键用被测 Address，与「SN 打向谁」一致，也限制 Validation 持票后对单一映射的发包频率。Inquire 限速键用控制通道前缀：收集窗由谁触发就限制谁，端口可随意更换故不能用完整 Addr。受托只保留 Target 去重表：表随挑战有效期过期，Equi-X 又把证明绑定到 Target，重放被限制在短窗口、固定目标、每目标一次。

## 影响 （Consequences）

- Spec 不得引入「Inquire 会话 ID」（把后续 `STUN:Cone` 绑回 Inquire 连接）或「按控制通道 IP 的 Live 会话」。Inquire 地址暂存（第 5 项）只约束速率，不是会话。
- 受托去重表与两张限速表的容量、驱逐策略由 Spec 规定，但不得改键。
...
- `DEC-0007`：Inquire 客户端地址暂存的键、30s 有效期与 `RateLimited`。
