...
当确认自己的 UDP 链路可用后，客户端从同一个本地端口向多个服务节点请求获取自身公网地址（`STUN:Addr`），检查返回结果：
...
返回地址与客户端本地数据包具体出口地址对比（IP:Port 全等比较）：
...
> 另外，需用不同的端口创建 UDPConn 监听，从根本上隔离，否则 `SN` 难以区分。

正式探测与预探测是各自独立的逻辑，与 NAT 映射的连续性没有关系。

> **For AI（Opus5）:**
> 也即：如果你预探测发现自己是 `Sym-Like`，也就没有必要再继续正式探测，而如果你不是 `Sym-Like`，正式探测就可以完全排除它。
> 这是一个很简单的自然逻辑。
...
一个源服务器通常需要（并行）委托 2~3 个受托服务器发送探测包。标准数量为**3**。源服务器会告知客户端具体的委托数量，这是一种友好的交互。
...
> **实现：**
> 受托服务器应当维护一个 `Address` 的缓存，有效期为时间片长度（2分钟）。
...
>
> 客户端尝试向不同服务器请求服务，返回的结果可能不同。客户端如实报告即可，这不在本协议的职责范围。
