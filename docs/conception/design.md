...
```go
// Rnd16: 16字节随机序列。
// Rnd16[0]: 首字节的低3位用于标识消息类型（Listen|NewPort|NewHost）。
// flag: 即 Listen | NewPort | NewHost 标识位
Rnd16[0] = Rnd16[0] & 0xF8 | flag

// TmpN: 变长随机字节序列，用于隐藏 SN 的长度特征。
// SN 的有效部分为 16 + 32 = 48 字节。
// 实现：Hash256 可以采用 BLAKE3-256 算法
...
1. Bas16：会话标识（SN）因子，16字节随机序列。**注**：规则同前 *哈希耦合*。
...
2. 在旧连接（裸UDP）上向该地址发送消息，数据内容同前 SN（见 `STUN:Cone` 会话标识小节）。
